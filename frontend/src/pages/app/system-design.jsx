...
function ChallengePicker({ onPick, onPickTemplate }) {
  ...
  const enrichedTemplates = useMemo(() => {
    ...
    return templates.map((t) => ({
      ...
      description:
        t.description ||
        `Prebuilt ${t.name} reference architecture. Tweak nodes, connect services, and simulate real load.`,
    }));
  }, []);
  ...
  const getChallengeCategories = useCallback((ch) => {
    ...
    if (ch.tags.includes("Feed") || ch.id === "social-feed" || ch.id === "ride-sharing")
      list.push("Social Media");
    if (ch.tags.includes("CDN") || ch.tags.includes("Encoding") || ch.id === "video-streaming")
      list.push("Streaming");
    if (
      ch.id === "ecommerce" ||
      ch.tags.includes("Catalog") ||
      ch.tags.includes("Orders") ||
      ch.tags.includes("Payments")
    )
      list.push("E-Commerce");
    if (ch.tags.includes("Search") || ch.id === "ecommerce") list.push("Search");
    if (
      ch.tags.includes("Database") ||
      ch.tags.includes("Storage") ||
      ch.id === "url-shortener" ||
      ch.id === "video-streaming" ||
      ch.id === "social-feed"
    )
      list.push("Storage");
    if (ch.tags.includes("CDN") || ch.id === "url-shortener" || ch.id === "video-streaming")
      list.push("CDN");
    return list;
  }, []);
  ...
  const stats = useMemo(() => {
    ...
    const free = enrichedChallenges.filter(
      (c) => c.difficulty === "Free" || c.difficulty === "Template",
    ).length;
    ...
    const notStarted = enrichedChallenges.filter(
      ...
      <div className="px-4 py-6 md:px-8 max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-4 lg:grid-cols-7 mb-6">
          ...
          <div className="rounded-xl border border-border bg-card p-4">
            ...
            <div className="mt-1 text-2xl font-bold text-emerald-500 font-mono">
              {stats.completed}
            </div>
          </div>
          <div className="rounded-xl border border-border bg-card p-4">
            <div className="text-xs text-muted-foreground">In Progress</div>
            <div className="mt-1 text-2xl font-bold text-amber-500 font-mono">
              {stats.inProgress}
            </div>
          </div>
          <div className="rounded-xl border border-border bg-card p-4">
            <div className="text-xs text-muted-foreground">Not Started</div>
            <div className="mt-1 text-2xl font-bold text-muted-foreground font-mono">
              {stats.notStarted}
            </div>
          </div>
        </div>
        ...
        {tab === "challenges" &&
          ...
          featuredChallenge && (
            <div className="mt-6">
              ...
              <div onClick={() => onPick(featuredChallenge)} className="group block cursor-pointer">
                  ...
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div className="space-y-2 flex-1">
                      <div className="flex items-center gap-2">
                        ...
                        <Badge
                          variant="outline"
                          className="border-primary/30 bg-primary/10 text-primary rounded-full font-mono text-[10px] uppercase tracking-wider"
                        >
                          Recommended Start
                        </Badge>
                      </div>
                    ...
                    <div className="flex flex-col justify-between border-t border-border pt-4 md:border-t-0 md:border-l md:pt-0 md:pl-6 md:min-w-[200px] text-xs text-muted-foreground space-y-2">
                      ...
                      <div className="flex justify-between items-center">
                        ...
                        <span className="font-semibold text-foreground flex items-center gap-1.5">
                          ...
                          {featuredChallenge.progress === "Not Started" && (
                      ...
                      <div className="flex justify-between items-center">
                        ...
                        <span className="font-semibold text-foreground">
                          {featuredChallenge.lastAttempted}
                        </span>
                      </div>
                    </div>
                  </div>
        ...
        {tab === "challenges" ? (
          <div className="mt-6 grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {displayChallenges.map((ch) => (
              <div key={ch.id} onClick={() => onPick(ch)} className="group block cursor-pointer">
                ...
                <Card className="h-full border-border bg-card p-5 transition-all hover:-translate-y-0.5 hover:border-primary/40 hover:shadow-md flex flex-col justify-between">
                  <div>
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <Badge
                        variant="outline"
                        className="border-border bg-muted/20 text-muted-foreground rounded-full font-mono text-[10px] uppercase tracking-wider"
                      >
                        Template
                      </Badge>
                    </div>
        ...
        {/* Empty State */}
        {((tab === "challenges" && sortedChallenges.length === 0) ||
          (tab === "practice" && filteredTemplates.length === 0)) && (
